...
const compile = () => {
  cachedCompilation ??= compileRoutes(config, ROUTES, '0'.repeat(64));
...
test('checked-in config and routes validate with fixed cubic and segment mappings', () => {
  const validated = validateSources(config, ROUTES);
  assert.deepEqual(validated.map(({ cubics }) => cubics.length), [15, 21, 15]);
  ...
  assert.deepEqual(harbor.segments.map(({ curveCount }) => curveCount), [1, 5, 2, 2, 4, 1]);
});
...
test('responsive projection preserves route-turn sign and emits bounded visual yaw', () => {
  ...
  for (const item of compile().schedules.filter((item) => item.route.id !== 'harbor-yard-rallycross')) {
...
test('generated serialization is deterministic, owned, precise and reset-stable', () => {
  const first = compile();
  const second = compileRoutes(config, ROUTES, '0'.repeat(64));
  ...
  assert.equal(
    ...
    'a4e5fcf468694b9894f46e651cb32add78143f525546c92e3665b1389e8b5bf3',
  );
});
...
test('generated slot selectors and every frame declaration have exact order and reset headings', () => {
  ...
  for (const track of ['ridge-pass', 'cypress-run', 'harbor-yard-rallycross']) {
    assert.deepEqual(
...
test('generated artifacts match the checked-in bytes', () => {
  const output = compileRoutes(config, ROUTES, sourceDigest([
    ...
    { path: 'routes/harbor-yard-rallycross.route.mjs', contents: readFileSync(new URL('../routes/harbor-yard-rallycross.route.mjs', import.meta.url)) },
  ]));
...
test('HTML and base CSS contain placeholders and preserve generated-first cascade ownership', () => {
  ...
  assert.doesNotMatch(baseCss, /@keyframes (?:ridge-pass|cypress-run|harbor-yard-rallycross)-traverse/);
  assert.doesNotMatch(baseCss, /animation(?:-name)?:\s*(?:ridge-pass|cypress-run|harbor-yard-rallycross)-traverse/);
